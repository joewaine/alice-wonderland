...
export class LevelBuilder {
  ...
  private async buildPlatforms(platforms: Platform[]): Promise<{ meshes: THREE.Mesh[], bouncy: BouncyPlatform[], breakable: BreakablePlatform[] }> {
    ...
    for (const platform of platforms) {
      ...
      if (platform.asset_id && hasGardenAsset(platform.asset_id)) {
        ...
        // Determine surface texture type from color
        const surfaceType = this.getSurfaceTextureType(color);
        const texture = getTexture(surfaceType);
        // Tile based on platform dimensions (1 repeat per ~4 world units)
        texture.repeat.set(platform.size.x / 4, platform.size.z / 4);
  ...
  /**
   * Determine texture type from platform color.
   * Green-ish → grass, brown/tan → stone, blue → marble, dark green → hedge.
   */
  private getSurfaceTextureType(color: THREE.Color): SurfaceTextureType {
    const r = color.r, g = color.g, b = color.b;

    // Dark green → hedge
    if (g > 0.25 && g > r * 1.5 && g > b * 1.5 && r < 0.4) return 'hedge';
    // Green dominant → grass
    if (g > r && g > b) return 'grass';
    // Brown/tan → stone
    if (r > 0.6 && g > 0.5 && b < g) return 'stone';
    // Blue-ish → marble
    if (b > r && b > 0.5) return 'marble';
    // Warm/earthy → wood
    if (r > g && r > b && g < 0.6) return 'wood';
